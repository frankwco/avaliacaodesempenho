...
public class IndicadorMB implements Serializable {
	...
	public void selecionarIndicador() {
		// ($1:@Valor com Pedágio;+$2:@Gastos com Combustível;+3)/2
	...
	public void verificarSmart() {
		...
		if (!smart.isTemporizavel()) {
			naoEe += " <li> Não é Temporizável </li>";
		}
		naoEe += " </ol>";
		// naoEe = naoEe.trim().replace(" ", "<br/>");

		if (naoEe.length() < 15) {
			mensagemNaoEe = "Recomendamos fortemente incluí-lo na Avaliação de Desempenho, o que você acha??";
			naoEe = "<ol>";
		...
		// O indicador ** foi classificado como naoE, deseja incluí-lo na
